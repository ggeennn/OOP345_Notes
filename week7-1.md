...
* **概念**：`<functional>` 库提供了可作为参数传递的**模板化函数对象（Function-Objects）**，常用于定制标准算法的行为。
* **用途**：代替 Lambda 表达式或完整自定义的函数对象类，实现如 `std::plus`, `std::greater` 等核心语言运算符的功能。

**2. C++ 引用（Reference）机制**

* **普通引用（`int&`）的不可重赋值性**：普通引用在初始化时就永久绑定到一个对象，不能被重新绑定到另一个对象。对引用的赋值操作实际上是修改其所绑定对象的值。
* **右值绑定限制**：普通引用（`int&`）不能绑定到右值（如字面量 `10`）。因为右值是临时的、不可寻址的。
* **常量引用（`const int&`）与右值**：`const int&` 可以绑定到右值，因为编译器会创建一个临时变量来存储右值，并将引用的生命周期延长，保证了安全。

**3. `std::reference_wrapper`**

* **核心作用**：将一个**引用（`T&`）** 包装成一个**可复制、可存储的对象**（`std::reference_wrapper<T>`），从而使引用能够安全地存入 `std::vector` 等标准容器中，同时保持对原始对象的引用行为。
* **`std::ref`**：是创建 `std::reference_wrapper` 的**辅助函数/工厂函数**，利用模板推导机制简化代码，避免手动指定冗长类型。
* **类型定义**：`std::vector<std::ref(...)>` 是错误的，因为 `std::vector` 模板需要**类型**，而 `std::ref(...)` 是一个**函数调用（值）**。

**4. `std::bind` 与闭包**

* **`std::bind` 返回类型**：`std::bind` 返回一个**未命名的闭包类型（Closure Type）**，该类型无法手写出完整名称，通常用 `auto` 接收。
* **闭包类型**：是编译器为实现特定逻辑（**行为 + 捕获的数据**）而自动生成的**匿名类**。
* **签名匹配原理**：`std::bind` 返回的可调用对象的签名（如 `int()`）由原始函数中**未被绑定**（即使用了 `std::placeholders`）的参数数量决定。如果所有参数都被固定，则签名变为 `()`。

**5. 迭代器（Iterator）分类**

* **六大分类**：`Input`, `Output`, `Forward`, `BiDirectional`, `RandomAccess`, `Contiguous`。
* **概念**：这些是迭代器的**能力等级**或**要求**，定义了迭代器支持的操作集合（如 `++`, `--`, 随机访问 `+` 等）。算法通过要求特定的迭代器类型来保证性能和正确性。

**6. 标准算法（Algorithms）**

* **`std::transform` (二元操作)**：
    * **作用**：对两个输入序列 $A$ 和 $B$ 逐元素应用二元操作，将结果存入输出序列 $C$。
    * **长度要求**：第二个输入序列 $B$ 的长度**必须**大于或等于第一个输入序列 $A$ 的长度（$\text{B.size()} \geq \text{A.size()}$），否则导致**未定义行为**。
    * **原地操作**：输出容器 $C$ 可以是输入容器 $A$ 或 $B$（原地操作是安全的）。
    * **返回值**：返回**输出序列 $C$ 中最后一个有效写入元素之后的迭代器位置**（即有效数据范围的末尾）。
* **`std::replace_if`**：
    * **逻辑**：遍历范围 `[f, l)`，如果元素满足 `predicate` 函数返回**真（true）** 的条件，则将其替换为新值 $t$。
* **`std::sort`**：
    * **比较器**：`Fn compare` 必须返回 `bool` 值，遵循**严格弱序**原则，表示“**第一个元素是否应该排在第二个元素的前面**”。
* **`std::merge`**：
    * **作用**：将两个**已经有序**的输入序列合并成一个更大的有序输出序列。
    * **排序要求**：输入序列必须按照 `std::merge` 使用的比较规则（默认升序）进行排序。
    * **返回值**：返回**输出序列 $C$ 中最后一个有效合并元素之后的迭代器位置**。若 $C$ 分配空间大于实际合并元素数量，返回的迭代器不等于 $\text{C.end()}$。