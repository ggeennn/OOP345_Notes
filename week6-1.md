...
2. 迭代器 (Iterators)
   - 核心: 作为容器与算法的“桥梁”，提供统一的遍历机制。
   - 行为: 像智能指针。支持 begin(), end(), ++, * (解引用)。
   - 嵌套类型: 属于容器类内部定义的类型别名 (例如: std::vector<T>::iterator)。

3. 算法 (Algorithms)
   - 核心: 定义在 <algorithm> 中，执行操作的通用函数模板 (如 std::sort, std::find)。
   - 依赖: 总是通过 **一对迭代器** ([begin, end)) 来定义操作范围。

4. 函数对象 (Function Objects / Functors)
   - 核心: 重载了 operator() 的类实例，行为像函数但可拥有状态。
   - 现代用法: 主要通过 **Lambda 表达式** 实现，用作算法的自定义规则 (如 std::sort 的比较谓词)。

5. 协作机制 (std::sort 示例)
   - std::sort(begin, end, predicate):
     - begin/end (迭代器): 告知算法操作范围。
     - predicate (二元谓词/Lambda): 必须返回 bool，定义严格弱序的比较规则。如果返回 true，表示第一个参数应排在第二个参数的前面 (例如 a < b 实现升序)。

==================================================
二、现代 C++ 语法：auto, Lambda, std::function
==================================================

1. auto 关键字
   - 类型推导 (Type Deduction): 让编译器自动确定变量类型。
   - 优势: 简化代码，尤其在处理复杂类型 (如迭代器或 Lambda 闭包类型) 时。
   - 推荐用法: 
     - **遍历**: for (const auto& item : container) (只读，避免复制)
     - **迭代器**: auto it = container.begin();

2. Lambda 表达式
   - 核心: 快速创建匿名函数对象 (本质是编译器生成的唯一闭包类型)。
   - 语法: [](params) { body }
   - **类型**: 必须使用 auto 来存储 Lambda (例如: auto my_lambda = [](int x){...};)。

3. std::function (来自 <functional>)
   - 核心: 多态函数包装器，用于**统一**存储和调用签名相同的各种可调用对象 (函数、Lambda、Functor)。
   - 语法: std::function<ReturnType(ArgTypes...)> func;
   - 价值: 允许接口与底层实现解耦，提高代码的灵活性和泛用性。

==================================================
三、std::string 与 视图/类型安全
==================================================

1. std::string 与 '\0'
   - 核心: std::string 是**类型安全**的字符容器，其长度由内部计数器 (size()) 决定。
   - 处理 '\0': std::string 可以安全地存储空字符 '\0'，它被视为普通字符。
   - C-Style 区别: C 风格字符串 (char*) **必须**以 '\0' 结束，C API (如 printf("%s")) 会在第一个 '\0' 处截断。

2. std::string_view (C++17)
   - 核心: **非拥有 (Non-owning)**、**只读 (Read-only)** 的字符串引用 (通常是 `const char*` + `size_t` 结构)。
   - 价值:
     - **零拷贝 (Zero-Copy)**: 创建视图或截取子字符串 (substr) 是 O(1) 操作，不涉及内存分配和数据复制。
     - **API 统一**: 可以接收 std::string, C-string literal, 或字符数组，且永远不会产生不必要的临时 std::string 对象。
   - 风险: string_view 不管理内存，必须确保其引用的原始数据**在其生命周期内保持有效**。

3. std::variant (C++17)
   - 核心: 类型安全的联合体，只能在声明的多种类型中存储**其中之一**。
   - 安全性: 始终知道自己存储的是哪个类型。
   - 访问: 
     - std::get<T>(v) 或 std::get<I>(v) 提取值。
     - 如果提取的类型或索引与当前存储的类型不匹配，会抛出 **std::bad_variant_access** 异常，防止未定义行为。

==================================================
四、交互规则与流程备忘
==================================================

- 模式: Learn | Assignment | Review
- 解释风格: Plain-first (先通俗解释，再给正式术语)。
- 交付物: 极简示例 (代码 ≤10 行)、1 道练习、标准答案 + 修正策略。
- STL 核心原则: **容器只管存，算法只管算** (通过迭代器解耦)。